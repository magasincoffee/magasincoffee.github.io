...
for(const width of [1280,768,390]){
 ...
 await check("ui2_011_"+width+"_action_center_canonical_counts_and_not_connected",async()=>{
   ...
   if(by.shift.state!=="ACTION_REQUIRED"||!by.shift.text.includes("2")||by.attendance.state!=="ACTION_REQUIRED"||!by.attendance.text.includes("1")||by.availability.state!=="READY"||!by.availability.text.includes("3")||by.schedule.state!=="READY"||!by.schedule.text.includes("4")||by.tasks.state!=="NOT_CONNECTED")throw new Error(JSON.stringify(rows));
...
await check("ui2_011_action_center_loading_error_empty_not_connected",async()=>{
 ...
 return JSON.stringify({loading,error,empty,notConnected});
});
