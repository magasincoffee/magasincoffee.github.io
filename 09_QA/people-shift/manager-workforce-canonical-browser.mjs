...
await check("manager_explicit_review",async()=>{
  ...
  const st=await page.evaluate(()=>globalThis.__MW31_QA.state.generation.status);
...
await check("manager_explicit_publish_emits_official_schedule",async()=>{
  ...
  const state=await page.evaluate(()=>({generation:globalThis.__MW31_QA.state.generation.status,official:globalThis.__MW31_QA.state.official.length}));
...
await check("official_workspace_reads_approved_schedule",async()=>{
  ...
  const rpc=await page.evaluate(()=>globalThis.__MW31_QA.calls.filter(x=>x.name==="get_manager_weekly_schedule").at(-1));
...
await check("store_filter_and_week_navigation_are_scoped",async()=>{
  ...
  await page.waitForFunction(()=>globalThis.__MW31_QA.calls.filter(x=>x.name==="get_manager_weekly_schedule").at(-1)?.args?.p_store_id==="store-a");
  const before=await page.evaluate(()=>globalThis.__MW31_QA.calls.filter(x=>x.name==="get_manager_weekly_schedule").at(-1)?.args?.p_week_start);
  await page.locator('[data-mos-week="next"]').click();
  await page.waitForFunction(prev=>globalThis.__MW31_QA.calls.filter(x=>x.name==="get_manager_weekly_schedule").at(-1)?.args?.p_week_start!==prev,before);
  const after=await page.evaluate(()=>globalThis.__MW31_QA.calls.filter(x=>x.name==="get_manager_weekly_schedule").at(-1)?.args?.p_week_start);
