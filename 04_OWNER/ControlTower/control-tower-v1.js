...
import { loadControlTowerSection } from "./source-isolation-v1.mjs";
...
async function applySourceSection(name, loader, fallback) {
  const section = await loadControlTowerSection(loader, fallback);
  ...
  await Promise.all([
    applySourceSection(
        ...
        }),
      {
        ...
        errorMessage:
          "Nguồn doanh thu đã đối chiếu tạm thời không khả dụng."
      }
    ),
    applySourceSection(
      "payables",
      () => loadProcurementPayables(core.supabase.get()),
      {
        source:
          "v_procurement_supplier_payables + v_procurement_order_summary",
        errorMessage: "Nguồn công nợ mua hàng tạm thời không khả dụng."
      }
    ),
    applySourceSection(
      "workforce",
      () => loadWorkforceAttention(core),
      {
        source:
          "get_manager_transfer_requests + list_schedule_generations",
        errorMessage: "Nguồn Workforce tạm thời không khả dụng."
      }
    )
  ]);
}
