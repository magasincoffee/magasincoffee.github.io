...
title MAGASIN NEW PC CONNECTOR V2

net session >nul 2>&1
if not "%errorlevel%"=="0" (
  echo [MAGASIN] Administrator access is required. Requesting Windows UAC approval...
  > "%TEMP%\magasin-elevate.vbs" echo Set UAC = CreateObject("Shell.Application")
  >> "%TEMP%\magasin-elevate.vbs" echo UAC.ShellExecute "%ComSpec%", "/c ""%~f0""", "", "runas", 1
  cscript.exe //nologo "%TEMP%\magasin-elevate.vbs" >nul 2>&1
  del /q "%TEMP%\magasin-elevate.vbs" >nul 2>&1
  exit /b 0
)
...
set "URL=https://raw.githubusercontent.com/magasincoffee/magasincoffee.github.io/85a9ba43ab911ba25175bb29c15d203d6dd91b73/08_INTEGRATIONS/supervisor/windows/connect-new-pc.ps1"
