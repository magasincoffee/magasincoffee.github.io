...
|---|---|---|---|---|---|---|---|---|
| Revenue — consolidated operating records | YES | dated operating rows verified through 2026-09-19 | date, branch CN1–CN4, direct/app indicators | MEDIUM-HIGH | ACTUAL | YES as reconciliation input | raw operating data is not yet repository-reconciled Revenue | define lineage in TASK-052; reconcile in TASK-053 |
...
| FoodApp provider settlement/fees | YES, partial | dated 2026 provider exports; latest identified export in this pass is 2026-08-05; no current-September provider settlement export verified | order, branch/store, completion state, gross, fee/discount/tax, net | HIGH for covered export | ACTUAL | YES for covered dates/provider | incomplete by date/provider | reconcile only matching covered periods; carry rest GAP |
...
| Payroll / labor | YES | attendance source contains dated entries through 2026-09-19; payroll/calculation sources can contain pre-entered rows, so row date alone does not prove worked/paid actuality | date, branch, hours, pay calculation | MEDIUM-HIGH scoped | ACTUAL only after attendance/period actuality gate | YES, bounded to verified actual attendance/pay period | current/open or pre-entered rows must not be treated as worked/paid actual without the actuality gate | separate attendance actuality from payroll calculation and preserve unclosed/unverified rows as GAP |
| Rent / Utilities | PARTIAL historical only | historical utility records exist; a current September 2026 branch P&L explicitly leaves rent unallocated and 2026 utilities unavailable for the covered period | some historical branch fields; current P&L gap markers | LOW current-period | GAP | NO for current period | active PFC period still lacks verified fixed-cost source | establish current rent/utilities evidence or approved allocation rule |
...
| Inventory consumption | PARTIAL | bounded CN3 evidence verified for 2026-09-01 through 2026-09-13 using opening + inbound - ending inventory | branch, period, material, opening, purchases, ending, consumption | MEDIUM-HIGH scoped | ACTUAL | YES only for proven scope | not company-wide; scope/period must remain attached | expand only when count/purchase coverage is proven |
...
A later read-only verification pass confirmed the same source-status map and tightened freshness semantics without reopening the task: operating Revenue rows are now verified through 2026-09-19; payroll/attendance sources contain dated rows through 2026-09-19 but still require actuality/period gates; current rent/utilities remain GAP after checking the current branch P&L and historical utility evidence; CN3 consumption is explicitly bounded to 2026-09-01 through 2026-09-13; the latest identified provider settlement export remains outside current September. File modified time is not treated as transaction coverage, and recipe file freshness remains separate from the recipe tab's own update date. No Drive write, locator, raw private record or synthetic financial value was introduced.
